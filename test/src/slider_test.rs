...
pub fn slider_tester() -> Html {
    ...
    let floats = html! {
        <>
        <Slider<f32> id="1" oninput={oninput.clone()} range={0.0..100.0} value={*value} step=1.0 fullwidth=true label={LabelAlignment::Right}/>
    ...
    let integers = html! {
        ...
        <Slider<u32> id="4" oninput={oninput.clone()} range={0..20} value={*value} step=1 fullwidth=true/>
        <p> {value.to_string()} </p>
        </>
    };
